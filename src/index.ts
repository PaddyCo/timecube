...
const getBests = async (userId: string, puzzleTypeSlug: string) => {
  ...
  const [single, ao5, ao12] = await Promise.all([
    prisma.time.findFirst({
      where: { ...timeWhere, milliseconds: { gt: 0 } },
      orderBy: [{ milliseconds: "asc" }, { performedAt: "asc" }],
    }),
    prisma.time.findFirst({
      where: { ...timeWhere, millisecondsAo5: { gt: 0 } },
      orderBy: [{ millisecondsAo5: "asc" }, { performedAt: "asc" }],
    }),
    prisma.time.findFirst({
      where: { ...timeWhere, millisecondsAo12: { gt: 0 } },
      orderBy: [{ millisecondsAo12: "asc" }, { performedAt: "asc" }],
    }),
  ]);
...
const getUser = async (userId: string, puzzleTypeSlug: string) => {
  ...
  const best = await getBests(userId, puzzleTypeSlug);
...
app.get(
  "/times/:puzzleTypeSlug/:userId",
  async (req: Request, res: Response) => {
    const [user, puzzleType] = await Promise.all([
      prisma.user.findFirst({ where: { id: req.params.userId } }),
      prisma.puzzleType.findFirst({
        where: { slug: req.params.puzzleTypeSlug },
      }),
    ]);
    if (!user) {
      res.status(404);
    ...
    const best = await getBests(user.id, puzzleType.slug);
    res.send({
      times,
      best,
    });
...
app.post(
  ...
  async (req: Request, res: Response) => {
    ...
    const calculateAvarage = (
      allTimes: { milliseconds: number; dnf: boolean }[]
    ): number | null => {
